/* ==========================================================================
  ...
  function registrarConsequenciaDaEspera(novoCard) {
    ...
    gameState.cardsAguardados.forEach(cardAguardado => {
      ...
      if (prioridadeNova < prioridadeAnterior && !cardAguardado.penalidadeAplicada) {
        ...
        gameState.pontuacao = Math.max(0, gameState.pontuacao + penalidade);
        atualizarPontuacaoUI();
  ...
  function agendarProximoCard(delay = 5000) {
    ...
    timeoutProximoCard = setTimeout(() => {
      timeoutProximoCard = null;
      if (gameState.emExecucao && firebaseGamePhase === 'official' && gameRulesAcknowledged && !isCurrentTabBlocked() && gameState.cardsAtivos.length === 0) {
  ...
  function gerarNovoCardIncidente() {
    ...
    registrarConsequenciaDaEspera(cardData);
    publicarEventoGameFirebase({
    ...
    cardElement.innerHTML = `
      ...
      <div class="game-card-options">
        ${card.opcoes.map((opt, idx) => `
          <button type="button" class="btn-option" data-card-id="${card.id}" data-option-index="${idx}">
            ${opt.texto}
          </button>
        `).join('')}
      </div>
      <div class="game-card-actions">
        <button type="button" class="btn-action-exec" data-card-id="${card.id}">⚡ Executar Ação</button>
        <button type="button" class="btn-action-wait" data-card-id="${card.id}">⏳ Aguardar</button>
  ...
  // Delegação de eventos: evita depender de onclick inline e garante que
  // opções e ações funcionem mesmo quando o card é criado dinamicamente.
  function configurarEventosCards() {
    const container = document.getElementById('game-cards-container');
    if (!container || container.dataset.eventsReady === '1') return;
    container.dataset.eventsReady = '1';
    container.addEventListener('click', (event) => {
      const option = event.target.closest('.btn-option');
      if (option && container.contains(option)) {
        selecionarOpcaoCard(option.dataset.cardId, Number(option.dataset.optionIndex), option);
        return;
      }
      const exec = event.target.closest('.btn-action-exec');
      if (exec && container.contains(exec)) {
        executarAcaoCard(exec.dataset.cardId);
        return;
      }
      const wait = event.target.closest('.btn-action-wait');
      if (wait && container.contains(wait)) {
        aguardarCard(wait.dataset.cardId);
      }
    });
  }
  ...
  function executarAcaoCard(cardId) {
    ...
    if (gameState.cardsAguardados.length) {
      setTimeout(() => retornarOcorrenciaAguardada() || agendarProximoCard(5000), 1500);
    } else {
      agendarProximoCard(5000);
    }
  }
...
function iniciarGameOficialLocal(sessionId) {
  ...
  console.info('[CCO 4.0] GAME OFICIAL recebido. Aguardando confirmação das regras. Sessão:', firebaseGameSessionId);
}


function mostrarRegrasGameOficial() {
  if (!gameState.operador || gameState.isAdmin || firebaseGamePhase !== 'official') return;
  let modal = document.getElementById('game-rules-modal');
  if (!modal) return;
  const sessionLabel = modal.querySelector('[data-session]');
  if (sessionLabel) sessionLabel.textContent = firebaseGameSessionId || 'Partida atual';
  const confirm = document.getElementById('btn-confirm-rules');
  if (confirm) confirm.disabled = false;
  modal.style.display = 'flex';
}

function confirmarRegrasGameOficial() {
  if (firebaseGamePhase !== 'official' || !gameState.emExecucao) return;
  ...
  agendarProximoCard(5000);
  publicarEventoGameFirebase({
    type: 'GAME_RULES_CONFIRMED',
    resultado: 'regras_confirmadas',
    points: 0,
    decisao: 'Operador confirmou leitura das regras.'
  });
  console.info('[CCO 4.0] Regras confirmadas. Primeiro card será liberado em 5 segundos.');
}
